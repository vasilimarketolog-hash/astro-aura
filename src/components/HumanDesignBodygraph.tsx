...
export const HumanDesignBodygraph: React.FC<HumanDesignBodygraphProps> = ({ data, locale }) => {
  ...
  const isDefined = (centerNameRu: string) => {
    return data.definedCenters.some((dc) =>
      dc.toLowerCase().includes(centerNameRu.toLowerCase().replace(' центр', ''))
    );
  };

  const selectedCenter = centers.find((c) => c.id === selectedCenterId) || centers[0];
  const selectedIsDefined = isDefined(selectedCenter.nameRu);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div>
            <div className="text-[11px] font-mono uppercase tracking-widest text-amber-800 font-bold mb-1">
              {locale === 'ru' ? 'Генетический Тип Дизайна Человека' : 'Human Design Genetic Type'}
            </div>
          ...
          <div className="px-4 py-2 rounded-2xl bg-amber-50 border border-amber-200 text-xs font-bold text-amber-900 shadow-xs">
            {locale === 'ru' ? 'Авторитет:' : 'Inner Authority:'} <span className="underline">{data.innerAuthority}</span>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-2">
          <div className="p-3 rounded-xl bg-stone-50 border border-stone-200">
            <span className="text-stone-500 block mb-0.5">{locale === 'ru' ? 'Стратегия жизни:' : 'Strategy:'}</span>
            <strong className="text-stone-900">{data.strategy}</strong>
          </div>
          <div className="p-3 rounded-xl bg-stone-50 border border-stone-200">
            <span className="text-stone-500 block mb-0.5">{locale === 'ru' ? 'Тема ложного Я:' : 'Not-Self Theme:'}</span>
            <strong className="text-rose-700">{data.notSelfTheme}</strong>
          </div>
          <div className="p-3 rounded-xl bg-stone-50 border border-stone-200">
            <span className="text-stone-500 block mb-0.5">{locale === 'ru' ? 'Определенность:' : 'Definition:'}</span>
        ...
        <div className="lg:col-span-6 bg-white border border-stone-200 rounded-3xl p-6 shadow-sm flex flex-col items-center select-none">
          <div className="text-center mb-4">
            <h3 className="text-base font-bold text-stone-900 flex items-center justify-center space-x-2">
              ...
              <span>{locale === 'ru' ? 'Интерактивная карта 9 Центров' : 'Interactive 9 Centers Bodygraph'}</span>
            </h3>
            <p className="text-xs text-stone-500">
              {locale === 'ru' ? 'Нажмите на любой центр для подробного разбора' : 'Click any center for energetic interpretation'}
            </p>
          </div>
          ...
          >
            ...
            {/* 9 Centers Rendering */}
            {centers.map((center) => {
              const defined = isDefined(center.nameRu);
          ...
          <div className="flex items-center space-x-4 text-xs mt-3 pt-3 border-t border-stone-100">
            <div className="flex items-center space-x-1.5">
              ...
              <span className="text-stone-700 font-medium">{locale === 'ru' ? 'Определенный' : 'Defined (Innate)'}</span>
            </div>
            <div className="flex items-center space-x-1.5">
              <span className="w-3.5 h-3.5 rounded-sm bg-white border border-stone-300 shadow-2xs" />
              <span className="text-stone-700 font-medium">{locale === 'ru' ? 'Открытый' : 'Open (Receptive)'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 bg-white border border-stone-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-5">
          <div className="flex items-center justify-between border-b border-stone-100 pb-4">
            <div>
              <span className="text-[11px] font-mono uppercase tracking-widest text-amber-800 font-bold">
                {locale === 'ru' ? 'Разбор энергетического центра' : 'Center Deep Dive'}
              </span>
              <h3 className="text-xl font-black text-stone-900">
                {locale === 'ru' ? selectedCenter.nameRu : selectedCenter.nameEn}
              </h3>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-amber-50/70 border border-amber-200 text-xs text-stone-800 space-y-2">
            <strong className="text-amber-950 font-bold flex items-center space-x-1.5">
              ...
              <span>{locale === 'ru' ? 'Практическая рекомендация:' : 'Actionable Advice:'}</span>
            </strong>
            <p>
              {selectedIsDefined
                ? locale === 'ru'
          ...
          <div className="space-y-2 pt-2">
            <h4 className="text-xs font-bold uppercase tracking-wider text-stone-500">
              {locale === 'ru' ? 'Врожденные суперсилы:' : 'Innate Superpowers:'}
            </h4>
